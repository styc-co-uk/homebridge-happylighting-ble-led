...
class Scanner extends EventEmitter {
  ...
  onDiscover(peripheral) {
    ...
    // change here
    const bulbServiceData = this.getValidServiceData(serviceData);
    if (!bulbServiceData) {
      return;
    }

    this.logPeripheral({ peripheral, serviceData: bulbServiceData });

    const result = this.parseServiceData(bulbServiceData.data);
    if (result == null) {
      return;
    }
